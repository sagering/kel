...
use crate::parser::{
    Operator, ParsedConst, ParsedExpression, ParsedForElement, ParsedFromImport, ParsedImport,
    ParsedListElement, ParsedModule, ParsedName, ParsedOneof, ParsedOperator, ParsedStatement,
    ParsedStringInterpolationPart, ParsedStruct, ParsedTemplate,
};
use regex::Regex;
...
pub fn type_name(context: &Context, type_id: TypeId) -> String {
    let typ = context.get_type(type_id);
        ...
        if type_is_generic(context, target) {
            ...
            if substitute(context, target, &substitutions).is_none() {
                Err(error!(
                    ...
                    type_name(context, target),
                    type_name(context, source.0),
                    source.1
                ))?
            };
...
pub fn check_module(
            ...
            ParsedStatement::FromImport(import) => {
                ...
                for import_name in names {
                    ...
                    for type_id in &module.types {
                        let type_name = type_name(context, *type_id);
    ...
    Ok(Module {
        ...
        types,
        templates,
        functions: Vec::new(), // functions cannot be defined in parsed modules
    })
}
